...
int config_mpu(void)
{
	int Status,Appu;
	//1
		Status=writeReg(MPUREG_PWR_MGMT_1,0x00);
		...
		usleep(100);
		//3
		Status=writeReg(MPUREG_CONFIG,0x01);
		if (Status != XST_SUCCESS) {
			return XST_FAILURE;
		}
		usleep(100);
		...
		usleep(100);
		//4
		Status=writeReg(MPUREG_GYRO_CONFIG,0xF8);
		if (Status != XST_SUCCESS) {
			return XST_FAILURE;
		}
		usleep(100);
		...
		usleep(100);
		//5
		Status=writeReg(MPUREG_ACCEL_CONFIG,0xF8);
		if (Status != XST_SUCCESS) {
			return XST_FAILURE;
		}
		usleep(100);
		...
		usleep(100);
		//6
		Status=writeReg(MPUREG_FIFO_EN,0xF8);
		if (Status != XST_SUCCESS) {
				return XST_FAILURE;
			}
		usleep(100);
		Appu=DynReadReg(MPUREG_FIFO_EN);
		usleep(100);
		//6
		...
		usleep(100);
		//7
		Status=writeReg(MPUREG_USER_CTRL,0x80);
		if (Status != XST_SUCCESS) {
			return XST_FAILURE;
		}
		usleep(100);
		...
		usleep(100);
		//9
		Status=writeReg(MPUREG_SIGNAL_PATH_RESET,0x0F);
		if (Status != XST_SUCCESS) {
			return XST_FAILURE;
		}
		usleep(100);
		Appu=DynReadReg(MPUREG_SIGNAL_PATH_RESET);
		usleep(100);
